...
typedef struct StromCmd__StatInfo
{
	...
	int64_t			tsc;		/* tsc counter */
	uint64_t		nr_ioctl_memcpy_submit;		/* MEMCPY_SSD2GPU_RAW or */
	uint64_t		clk_ioctl_memcpy_submit;	/* MEMCPY_SSD2GPU_BLOCK */
	uint64_t		nr_ioctl_memcpy_wait;		/* MEMCPY_WAIT */
	uint64_t		clk_ioctl_memcpy_wait;
	uint64_t		clk_setup_iov;
	uint64_t		clk_setup_file;			/* filesystem get_block() */
	uint64_t		clk_setup_raid;			/* raid0 mapping */
	uint64_t		clk_setup_prps;			/* allocation of PRP/SGList */
	uint64_t		clk_submit_dma;			/* submit requests */
	uint64_t		clk_submit_wait;		/* wait by limiter */
	uint64_t		nr_ssd2gpu_dma;			/* # of DMA execution */
	uint64_t		clk_ssd2gpu_dma;		/* h/w exec time of DMA */
	uint64_t		clk_ram2gpu_copy;		/* page-cache to GPU */
